 ...
 * based on the Mounjaro Japones principles.
 *
...
const MealSuggestionOutputSchema = z.object({
  ...
  whyItWorks: z.string().describe('An explanation of why this meal helps with weight loss and pairs well with the Mounjaro Japones tea.'),
});
...
const mealSuggestionPrompt = ai.definePrompt({
    ...
    prompt: `Você é um chef e nutricionista especialista na filosofia Mounjaro Japonês, que combina chás metabólicos com uma alimentação saudável para perda de peso.
...
Princípios da Filosofia Mounjaro Japonês:
...
5. Em "whyItWorks", explique de forma motivacional como esta refeição ajuda na perda de peso e se alinha com a filosofia Mounjaro Japonês, mencionando como ela potencializa os benefícios dos chás. Conecte a sugestão ao sonho do usuário.
